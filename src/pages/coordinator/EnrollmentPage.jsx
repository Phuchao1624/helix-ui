...
export default function EnrollmentPage() {
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r) => <button className="icon-button" aria-label={`Mở thao tác ghi danh cho ${r.name}`}><MoreHorizontal size={18} /></button> },
  ];
  return <>
    ...
    <Section><div className="toolbar"><SearchField value={query} onChange={setQuery} placeholder="Tìm học sinh…" /><select aria-label="Lọc theo lớp"><option>Lớp A2-01</option><option>Lớp A2-02</option><option>Lớp B1-01</option></select></div><Tabs items={['All', 'Active', 'Pending', 'Transferred']} active={tab} onChange={setTab} /><DataTable columns={columns} rows={rows} /></Section>
