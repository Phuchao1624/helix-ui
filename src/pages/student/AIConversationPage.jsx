...
const coaches = {
  luna: { name: 'Luna', label: 'Giọng nữ', accent: '#8b6cff', reply: 'That sounds fun! What are you most looking forward to this weekend?' },
  leo: { name: 'Leo', label: 'Giọng nam', accent: '#1b9bd7', reply: 'Nice plan! Who would you like to spend the weekend with?' },
};
...
export default function AIConversationPage() {
  ...
  const beginListeningTurn = () => {
    ...
    setSpeaking(false); setListening(true); setCaption('Mình đang nghe bạn nói…');
    const heard = window.setTimeout(() => {
      if (!callActiveRef.current || pausedRef.current) return;
      setListening(false); setCaption("I'm planning to visit a new café and hang out with my friends.");
  ...
  const togglePause = () => {
    ...
    setSpeaking(false); setListening(false); setCaption(next ? 'Cuộc hội thoại đang tạm dừng.' : 'Tiếp tục nhé — mình đang nghe bạn.');
  ...
  return <main className="ai-call">
    <header className="ai-call__titlebar"><button type="button" onClick={() => navigate('/student/ai')} aria-label="Quay lại AI Practice"><ArrowLeft size={20} /></button><div><h1>Speaking with AI</h1><p>Luyện nói trực tiếp với AI</p></div><Button variant="secondary" onClick={() => navigate('/student/ai')}>Kết thúc buổi học</Button><div className="token-balance"><Sparkles size={17} /><strong>{tokenBalance}</strong><span>tokens</span></div></header>

    <section className="ai-call__summary" aria-label="Thông tin phiên học"><div><SessionIcon type="topic" /><span><small>Chủ đề</small><strong>Weekend Plans</strong></span></div><div><SessionIcon type="level" /><span><small>Cấp độ</small><strong>Intermediate B1</strong></span></div><div><SessionIcon type="time" /><span><small>Thời gian</small><strong>{time}</strong></span></div><div><SessionIcon type="connection" /><span><small>Kết nối</small><strong>Tốt</strong></span></div><div><SessionIcon type="ai" /><span><small>HELIX AI</small><strong>{coaches[coach].name}</strong></span></div><div><SessionIcon type="session" /><span><small>Phiên học</small><strong>#240521</strong></span></div></section>

    <div className="ai-call__workspace">
      <aside className="ai-call__brief"><h2><Target size={17} /> Bối cảnh &amp; Mục tiêu</h2><dl><div><dt>Chủ đề</dt><dd>Weekend Plans</dd></div><div><dt>Mục tiêu</dt><dd>Luyện nói về kế hoạch cuối tuần, sử dụng thì tương lai và từ nối.</dd></div></dl><section><h3><Lightbulb size={15} /> Gợi ý nhanh</h3>{["I'm planning to…", 'I might… if the weather is good.', "One thing I'm really looking forward to is…"].map((hint) => <button key={hint} type="button" onClick={() => { setCaption(hint); setSpeaking(false); }}><span>{hint}</span><Volume2 size={14} /></button>)}</section><section><h3><BookOpen size={15} /> Từ vựng hữu ích</h3><div className="ai-call__vocab">{['plan', 'hang out', 'outdoor', 'excited', 'relax', 'schedule', '+12'].map((word) => <span key={word}>{word}</span>)}</div></section><div className="ai-call__note"><Sparkles size={16} /><p><strong>Mẹo nhỏ</strong>Hãy nói đầy đủ ý, dùng từ nối để bài nói mạch lạc hơn.</p></div></aside>
      ...
      <section className="ai-call__stage">
        ...
        <div className={`ai-call__state ${speaking ? 'is-speaking' : listening ? 'is-listening' : ''}`}><span>{Array.from({ length: 5 }, (_, index) => <i key={index} />)}</span>{speaking ? 'AI đang nói…' : listening ? 'AI đang nghe…' : 'Sẵn sàng lắng nghe'}</div>
        <div className={`ai-call__caption ${captions ? '' : 'is-hidden'}`}><p>{caption}</p><label><Captions size={15} /> Phụ đề AI <input type="checkbox" checked={captions} onChange={(event) => setCaptions(event.target.checked)} /></label></div>
        <div className="ai-call__coaches"><span>Chọn giọng AI</span><div>{Object.entries(coaches).map(([key, item]) => <button type="button" key={key} className={coach === key ? 'is-active' : ''} style={{ '--coach-accent': item.accent }} onClick={() => chooseCoach(key)}><span className={`coach-face coach-face--${key}`}><i /><b /></span><span><strong>{item.name}</strong><small>{item.label}</small></span>{coach === key ? <Check size={13} /> : null}</button>)}</div></div>
      </section>

      <aside className="ai-call__progress"><h2>Tiến trình buổi học</h2><div className="ai-call__completion"><span><small>Thời gian</small><strong>{time}</strong></span><div><b>65%</b></div><small>Đã hoàn thành</small></div><section><header><h3>Đánh giá hiện tại</h3><strong>8.2<small>/10</small></strong></header>{[['Phát âm',85],['Trôi chảy',80],['Từ vựng',85],['Ngữ pháp',78]].map(([label, value], index) => <div className="ai-score-row" key={label}><ScoreIcon index={index} /><div className="ai-score-row__metric"><span>{label}<b>{(value / 10).toFixed(1)}</b></span><div><i style={{ width: `${value}%`, '--score-color': index === 3 ? '#ffab3e' : index === 1 ? '#1b9bd7' : '#2ec27e' }} /></div></div></div>)}</section><section className="ai-call__goals"><h3>Mục tiêu</h3><span><Check size={14} /> Giữ cuộc hội thoại ít nhất 5 phút</span><span><Check size={14} /> Sử dụng 10 từ vựng mới</span><span><i /> Phát âm rõ ràng &gt; 85%</span></section></aside>
    </div>

    <footer className="ai-call__controls"><button type="button" onClick={() => setCaptions(!captions)} className={captions ? 'is-active' : ''}><Captions size={18} /><span>Phụ đề AI</span></button><button type="button" onClick={togglePause} disabled={!callActive}>{paused ? <Play size={18} /> : <Pause size={18} />}<span>{paused ? 'Tiếp tục' : 'Tạm dừng'}</span></button><button type="button" onClick={() => setCaption("Try: I'm really looking forward to…")}><Lightbulb size={18} /><span>Gợi ý</span></button><button type="button" className={`ai-call__mic ${listening ? 'is-listening' : ''} ${callActive ? 'is-active' : ''}`} onClick={startConversation}><span><Mic2 size={28} /></span><b>{!callActive ? 'Bắt đầu hội thoại' : paused ? 'Đang tạm dừng' : listening ? 'Bạn đang nói…' : speaking ? 'AI đang trả lời…' : 'Cuộc gọi đang diễn ra'}</b></button><button type="button" onClick={() => { clearConversationTimers(); makeCoachSpeak(); }} disabled={!callActive}><RotateCcw size={18} /><span>Phát lại</span></button><button type="button"><Settings2 size={18} /><span>Cài đặt</span></button><button type="button" className="is-danger" onClick={() => { callActiveRef.current = false; clearConversationTimers(); navigate('/student/ai'); }}><span>●</span><b>Kết thúc</b></button></footer>
  </main>;
}
