...
export default function AssignmentsPage() {
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: (r) => <button className="icon-button" aria-label={`Mở thao tác cho ${r.title}`}><MoreHorizontal size={18} /></button> },
  ];
  return <>
    ...
    <Section><div className="toolbar"><select aria-label="Lọc theo lớp"><option>Tất cả lớp</option><option>A2-01</option><option>A2-02</option></select><select aria-label="Sắp xếp bài tập"><option>Hạn nộp gần nhất</option><option>Mới tạo</option></select></div><Tabs items={['All', 'Draft', 'Active', 'Grading', 'Returned']} active={tab} onChange={setTab} /><DataTable columns={columns} rows={rows} /></Section>
