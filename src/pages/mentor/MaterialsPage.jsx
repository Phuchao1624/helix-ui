...
export default function MaterialsPage() {
  ...
  const columns = [
    ...
    { key: 'download', label: '', render: (r) => <div className="table-actions"><button className="icon-button" aria-label={`Tải ${r.name}`}><Download size={17} /></button><button className="icon-button" aria-label={`Mở thao tác cho ${r.name}`}><MoreHorizontal size={17} /></button></div> },
  ];
  return <>
    ...
    <Section><div className="toolbar"><SearchField value={query} onChange={setQuery} placeholder="Tìm theo tên tệp…" /><select aria-label="Lọc theo khoá học"><option>Tất cả khoá học</option><option>Everyday English A2</option><option>Communication Skills B1</option></select><select aria-label="Lọc theo loại tệp"><option>Tất cả loại tệp</option><option>PDF</option><option>Audio</option><option>Document</option></select></div><DataTable columns={columns} rows={rows} /></Section>
