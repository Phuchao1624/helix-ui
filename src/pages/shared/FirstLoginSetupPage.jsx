  ...
  return (
    <AuthLayout title="Chào mừng bạn đến với HELIX." note="Hoàn tất một bước bảo mật trước khi bắt đầu hành trình học tập.">
      ...
      <form className="auth-form" onSubmit={(e) => { e.preventDefault(); navigate('/student'); }}>
        ...
        <Field label="Mật khẩu mới" required hint="Tối thiểu 8 ký tự, gồm chữ hoa, chữ thường và số."><div className="input-with-icon"><input type={show ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} /><button type="button" onClick={() => setShow(!show)} aria-label={show ? 'Ẩn mật khẩu' : 'Hiện mật khẩu'}>{show ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></Field>
