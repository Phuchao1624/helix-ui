...
export function Progress({ value, label, compact = false }) { const safeValue = Math.min(100, Math.max(0, value)); return <div className={`progress ${compact ? 'progress--compact' : ''}`}>{label ? <div className="progress__meta"><span>{label}</span><strong>{safeValue}%</strong></div> : null}<div className="progress__track" role="progressbar" aria-label={label || 'Tiến độ'} aria-valuemin="0" aria-valuemax="100" aria-valuenow={safeValue}><span style={{ width: `${safeValue}%` }} /></div></div>; }
...
export function Tabs({ items, active, onChange, label = 'Bộ lọc nội dung' }) {
